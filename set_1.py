...
#Below is my initial code that generates a random number for the parameters between a hypothetical defined range in Pesos
'''import math
import random
def savings():
...
def material_waste(total_material, material_units, num_jobs, job_consumption):
    ...
    To get the waste of a set of jobs:
        1. Multiply the number of jobs by the material consumption per job. (material_consumed=num_jobs*job_consumption)
    ...
    wasted_material=total_material-int(material_consumed)
    result=str(wasted_material)+material_units
    return str(result)
